...
const applyGroupOffers = (basket, subtotal, itemsForGroupOffer) => {
    itemsForGroupOffer.sort((a, b) => {
        if (pricelistAndOffers_1.priceMapping[a].price > pricelistAndOffers_1.priceMapping[b].price)
            return 1;
        if (pricelistAndOffers_1.priceMapping[b].price > pricelistAndOffers_1.priceMapping[a].price)
            return -1;
        return 0;
    });
    while (itemsForGroupOffer.length >= 3) {
        for (let i = 0; i < 3; i++) {
            itemsForGroupOffer.pop();
        }
        subtotal += 45;
    }
